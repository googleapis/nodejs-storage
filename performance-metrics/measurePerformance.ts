...
async function main() {
  ...
  await generateResultCSV(results);
}
...
function generateRandomFileName(currentIteration: number): string {
  return `${TEST_NAME_STRING}.${currentIteration}.${uuid.v4()}`;
}
...
async function generateResultCSV(results: TestResult[]) {
  const csv = results.map(result => Object.values(result));
  csv.unshift(Object.keys(results[0]));
  const csvString = csv.join('\n');
  await writeFile(`nodejs-perf-metrics-${Date.now().toString()}.csv`, csvString);
}
